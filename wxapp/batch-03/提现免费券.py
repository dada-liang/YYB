...
YYB_SERVER = os.environ.get("YYB_SERVER", "")
YYB_TIMEOUT = 40
...
def parse_account(raw: str) -> tuple[str, str, str]:
    ...
    server, separator, openid = account.rpartition("@")
    if not separator or not server.strip() or not openid.strip():
        raise ClaimError("YYB_SERVER 格式无效")
    server = server.strip().rstrip("/")
    if not server.lower().startswith(("http://", "https://")):
        server = f"http://{server}"
    return server, openid.strip(), remark.strip()


def getjscode(server: str, openid: str) -> list[str]:
    url = f"{server}/wxapp/getCode"
    params = {"ref": openid, "app_id": APPID}
    try:
        resp = requests.post(
            url=url,
            json=params,
...
def main() -> int:
    ...
    accounts = [item.strip() for item in YYB_SERVER.replace("\r", "\n").replace("\n", "&").split("&") if item.strip()]
    if not accounts:
        print("未配置 YYB_SERVER，格式：yyb-go:8000@账号ID或OpenID")
        return 1
    ...
    for index, raw in enumerate(accounts, start=1):
        prefix = f"🌸 账号[{index}]"
        try:
            server, openid, remark = parse_account(raw)
            if remark:
                prefix += f"[{remark}]"
            result = run_account(server, openid)
...
def run_account(server: str, openid: str) -> dict[str, Any]:
    track_id = make_track_id()
    session = requests.Session()
    session.verify = False

    codes = getjscode(server, openid)
    if not codes:
