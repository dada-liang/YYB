...
class Task {
    constructor(env) {
        ...
        this.isLegacyToken =
            at <= 0 && this.user.length >= 2 && /^ey[A-Za-z0-9_-]{10,}\./.test(this.accountId);
    ...
    async prepareToken() {
        ...
        if (!PHONE_LOGIN) {
            $.log(`账号[${this.index}] 未开启手机号授权，无法自动完成首次登录`);
            return false;
        }
    ...
    cacheKey() {
        return this.isLegacyToken ? this.accountId : yybCacheKey(this.server, this.accountId);
    }
