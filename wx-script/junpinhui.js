...
变量名：junpinhui
变量值：YYB服务器地址@账号ID或OpenID，多账号用 & 或换行分隔
      也支持 服务器地址@账号#token 或仅 token（手动token入口，JSON 仍支持）
依赖变量：YYB_SERVER（服务器地址@账号ID或OpenID）
可选变量：junpinhui_encrypt_payload —— YYB /wx/encryptkey 需要小程序真实完整
      payload(JSON，抓包 getUserEncryptKey)；不配则仅影响用 encryptData 的农场操作
...
// YYB /wx/encryptkey 要求调用方提供真实 payload（旧 wx_server 的服务端代生成密钥方式 YYB 不提供）
const ENCRYPT_PAYLOAD_RAW = String(process.env.junpinhui_encrypt_payload || "").trim();
...
function parseAccount(raw) {
  ...
  if (!text) return { openid: "", token: "", server: "" };

  // YYB 格式：服务器地址@账号[#token]；仅对非 JSON 文本做 @ 拆分
  let server = "";
  let body = text;
  if (!text.startsWith("{")) {
  ...
  for (const sep of ["#", "|"]) {
    if (body.includes(sep)) {
      const [openid, ...rest] = body.split(sep);
      return { openid: openid.trim(), token: rest.join(sep).trim(), server };
    }
  }

  if (body.length > 40 && !body.startsWith("o")) return { openid: "", token: body, server };
  return { openid: body, token: "", server };
}
...
class Task {
  constructor(raw) {
    ...
    // 手动 token 账号沿用原缓存键（openid 或 token 摘要）；YYB 自动登录账号用 server@ref
    this.cacheKey = this.openid && this.token
      ? this.openid
      : this.openid && this.server
        ? yybCacheKey(this.server, this.openid)
        : this.openid || (this.token ? md5(this.token).slice(0, 16) : `account_${this.index}`);
    this.wechat = this.server ? new WeChatServer({ url: this.server, appid: MINI_APP_ID, auth: process.env.wx_auth || "" }) : null;
    // garden 侧要用习酒 appid 取 code / 取加密密钥
    this.gardenWechat = this.server ? new WeChatServer({ url: this.server, appid: GARDEN_APP_ID, auth: process.env.wx_auth || "" }) : null;
    this.member = {};
  }
  ...
  async getWxCode() {
    if (!this.wechat) throw new Error("YYB_SERVER 格式无效（应为 服务器地址@账号ID或OpenID），无法自动登录");
    const { data } = await this.wechat.getCode(this.openid);
   ...
   */
  async gardenLogin() {
    if (!this.gardenWechat) throw new Error("YYB_SERVER 格式无效（应为 服务器地址@账号ID或OpenID），无法登录 garden");
    const pick = (d) => d?.data?.code || d?.code;

    const r1 = await this.gardenWechat.getCode(this.openid);
    ...
    const r2 = await this.gardenWechat.getCode(this.openid);
   ...
   */
  async getEncryptKey() {
    if (!this.gardenWechat) throw new Error("YYB_SERVER 格式无效（应为 服务器地址@账号ID或OpenID），无法生成 encryptData");
    if (!ENCRYPT_PAYLOAD_RAW) {
      throw new Error("缺少 junpinhui_encrypt_payload：YYB /wx/encryptkey 需要小程序真实完整 payload(JSON)，旧 wx_server 的服务端代生成密钥方式 YYB 不提供");
    }
    let payload;
    try {
      payload = JSON.parse(ENCRYPT_PAYLOAD_RAW);
    } catch (e) {
      throw new Error(`junpinhui_encrypt_payload 不是有效 JSON: ${e.message || e}`);
    }
    if (!payload || typeof payload !== "object") throw new Error("junpinhui_encrypt_payload 必须是 JSON 对象");
    const { data } = await this.gardenWechat.getEncryptKey(this.openid, payload);
    const result = data.data || {};
    let raw = result.raw || {};
    if (typeof raw === "string") { try { raw = JSON.parse(raw); } catch { raw = {}; } }
    for (const item of [result, raw]) {
      const encryptKey = item.encrypt_key || item.encryptKey;
      const iv = item.iv;
      const version = item.version;
      if (encryptKey && iv && version !== undefined) return { encryptKey, iv, version };
    }
    throw new Error(`YYB /wx/encryptkey 未返回 encryptKey/iv/version: ${JSON.stringify(result)}`);
  }
