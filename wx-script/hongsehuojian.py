...
# 说明: 通过 YYB(/wxapp/getCode) 用账号自动登录, 完成每日签到
# 账号来源: 环境变量 YYB_SERVER (格式: 服务器地址@账号ID或OpenID, 多账号用换行或 & 分割, 可选 #备注)
# 可选变量:
#   hshj_ticket       手动 ticket, 格式 token 或 token#userId, 多账号用换行/& 分割(按账号顺序)
#   hshj_phone_login  是否允许用 YYB /wxapp/getPhoneNumber 自动登录, 默认 1(开启), 置 0 关闭
#   hshj_encrypt_payload  getUserEncryptKey 的真实完整 payload(JSON), 不得编造
...
ENCRYPT_PAYLOAD_RAW = os.getenv("hshj_encrypt_payload", "").strip()
...
def response_candidates(payload):
...
def yyb_post(account, path, payload):
    headers = {"Content-Type": "application/json"}
    resp = session.post(
        f"{account['server']}{path}", json=payload,
        headers=headers, timeout=60,
    )
    resp.raise_for_status()
    data = resp.json()
    if not isinstance(data, dict) or data.get("code") != 0:
        raise RuntimeError(f"YYB请求失败: {data.get('msg') if isinstance(data, dict) else data}")
    return data
...
# smallcat
# ---------------------------------------------------------------------------
def get_code(account_id, phone=False):
    """smallcat 偶发 "获取失败"(运行时会话抖动), 刷新会话后间隔重试。"""
    last_msg = ""
    for attempt in range(4):
        if attempt:
            time.sleep(3)
        path = "/wxapp/getPhoneNumber" if phone else "/wxapp/getCode"
        data = yyb_post(account_id, path, {"ref": account_id["ref"], "app_id": MINI_APP_ID})
        for item in response_candidates(data):
            code = item.get("code") or item.get("phoneCode") or item.get("phone_code")
...
def get_encrypt_key(account_id):
    if not ENCRYPT_PAYLOAD_RAW:
        raise RuntimeError("缺少 hshj_encrypt_payload；YYB /wx/encryptkey 要求原小程序真实完整 payload")
    try:
        payload = json.loads(ENCRYPT_PAYLOAD_RAW)
    except ValueError as exc:
        raise RuntimeError(f"hshj_encrypt_payload 不是有效 JSON: {exc}") from exc
    if not isinstance(payload, dict):
        raise RuntimeError("hshj_encrypt_payload 必须是 JSON 对象")
    data = yyb_post(account_id, "/wx/encryptkey", {
        "ref": account_id["ref"], "app_id": MINI_APP_ID, "payload": payload,
    })
    for item in response_candidates(data):
        encrypt_key = item.get("encrypt_key") or item.get("encryptKey")
        if encrypt_key:
            return encrypt_key, item.get("version")
    return None, None
...
class RedRocket:
    def __init__(self, account_id, index):
        self.yyb_account = account_id
        self.account_id = account_cache_key(account_id)
        self.index = index
        self.ticket = ""
        self.user_id = ""
        self.openid = ""
    ...
    def request(self, path, data=None, method="POST", _retried=False):
        ...
        if code == "7005" and not _retried:
            ...
            self.encrypt_key, self.encrypt_ver = get_encrypt_key(self.yyb_account)
    ...
    def resolve_openid(self):
        ...
        code = get_code(self.yyb_account)
    ...
    def login_by_phone(self):
        ...
        phone_code = get_code(self.yyb_account, phone=True)
        body = {
...
def run_account(account_id, index):
    ...
    client.encrypt_key, client.encrypt_ver = get_encrypt_key(client.yyb_account)
...
def main():
    ...
    for x in raw.replace("&", "\n").splitlines():
        ...
        try:
            ...
            print(f"❌ {e}")
    if not accounts and os.getenv("hshj", "").strip():
        print("❌ 检测到旧变量 hshj：本脚本账号来源已改为环境变量 YYB_SERVER（格式：服务器地址@账号ID或OpenID），请迁移配置。")
    if not accounts:
