...
class Task {
    ...
    async getCode() {
        const res = await axios.post(`${this.account.server}/wxapp/getCode`, { ref: this.account.openid, app_id: MINI_APP_ID }, {
    ...
    async getPhoneCode() {
        try {
            const res = await axios.post(`${this.account.server}/wxapp/getPhoneNumber`, { ref: this.account.openid, app_id: MINI_APP_ID }, {
    ...
    async run() {
        if (!this.account.server || !this.account.openid) { this.log("跳过：YYB_SERVER 格式无效"); return; }
        try {
