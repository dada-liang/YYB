...
# 说明: 通过 YYB(/wxapp/getCode) 用账号换取 wx.login code 自动登录, 完成每日签到
# 账号来源: 环境变量 YYB_SERVER (格式: 服务器地址@账号ID或OpenID, 多账号用换行或 & 分割, 可选 #备注)
# 需要配置: YYB_SERVER, 用于获取 wx.login code
#new Env("芯享会签到")
#cron 49 15 * * *
...
def get_cached_token(account):
    return (read_token_cache().get(account_cache_key(account)) or {}).get("access_token")


def save_cached_token(account, access_token):
    cache = read_token_cache()
    cache[account_cache_key(account)] = {"access_token": access_token, "updatedAt": int(time.time())}
    write_token_cache(cache)


def remove_cached_token(account):
    cache = read_token_cache()
...
def extract_yyb_code(payload):
    ...
    data = payload.get("data") if isinstance(data, dict) else None
...
def login(account):
    ...
    if rcode in NEED_AUTH_CODES or "授权" in msg or "未注册" in msg:
        ...
        code2 = get_wx_code(account)  # code 单次有效, 重新获取
...
def get_token_for_account(account, index, force=False):
    if not force:
        cached = get_cached_token(account)
        if cached:
            print(f"账号 {index} 使用缓存 token: {mask(cached)}")
            return cached
    access_token, nick = login(account)
    save_cached_token(account, access_token)
...
def run_account(account, index):
    ...
    token = get_token_for_account(account, index)
    ...
    if int(state.get("code", -1)) == SESSION_INVALID_CODE:
        ...
        remove_cached_token(account)
        token = get_token_for_account(account, index, force=True)
        state = sign_in_list(token)
...
def main():
    ...
    for x in raw.replace("&", "\n").splitlines():
        ...
        try:
            ...
            print(f"❌ {e}")
    if not accounts and os.getenv("xxh", "").strip():
        print("❌ 检测到旧变量 xxh：本脚本账号来源已改为环境变量 YYB_SERVER（格式：服务器地址@账号ID或OpenID），请迁移配置。")
    if not accounts:
