...
#Notice:
变量名：wx_xlxyh
变量值：YYB服务器地址@账号ID或OpenID，多账户用 & 或换行；支持 openid#备注
...
class Task {
    constructor(env) {
        ...
        this.yyb = parseYYBAccount(this.raw);
        if (this.yyb) {
            this.accountId = this.yyb.ref;
            this.remark = this.yyb.remark;
        } else {
            this.accountId = (this.user[0] || "").trim();
            this.remark = (this.user[1] || "").trim();
        }
        this.cacheId = this.yyb ? yybCacheKey(this.yyb.server, this.yyb.ref) : this.accountId;
     ...
     */
    async getServerCode() {
        if (!this.wechat) throw new Error("YYB_SERVER 格式无效（应为 服务器地址@账号ID或OpenID），无法取 code");
        const { data } = await this.wechat.getCode(this.accountId);
    ...
    async login(force = false) {
        ...
        const cached = this.cache[this.cacheId];
...
!(async () => {
    ...
    if (!$.userCount) {
        $.log(`未找到变量【YYB_SERVER 或 ${ckName}】：每行填 服务器地址@账号ID或OpenID，多账号用 & 或换行`);
        return;
    }
