...
登录（纯 wx code，无需小米账号密码/passToken 外部凭证）：
  0) 取微信 getUserInfo 加密资料：原脚本走 wx_server/wx/getuserinfo {appid,openid}
       -> data.{cloud_id,encryptedData,iv,signature,data(明文profile)}
       （YYB 版：YYB 不提供 getUserInfo 加密数据，此资料为空，按原脚本缺失分支处理）
...
class Task {
    ...
    // 注意：YYB 不提供 getUserInfo 的加密数据（/wx/getuserinfo 只回普通资料），这里如实返回 null，
    // 登录链路按原脚本的缺失分支处理；若小米侧强制要求该 cookie，则该号无法自动登录并如实报错。
    ...
    async login() {
        ...
        if (r2.status >= 300 && r2.status < 400) {
            ...
            cache[this.cacheId] = { ...session, updatedAt: new Date().toISOString() };
            writeCache(cache);
        }
