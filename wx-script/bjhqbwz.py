...
#       连续签达标后会获得抽奖机会, 但抽奖属于领奖动作, 本脚本不自动触发, 仅完成签到。
...
def get_cached_entity(account_id):
    return read_token_cache().get(account_cache_key(account_id)) or {}


def save_cached_entity(account_id, entity):
    cache = read_token_cache()
    cache[account_cache_key(account_id)] = {
...
def get_wx_code(account_id):
    body = {"ref": account_id["ref"], "app_id": MINI_APP_ID}
    last_msg = ""
    for attempt in range(4):
        if attempt:
            time.sleep(3)
        resp = session.post(f"{account_id['server']}/wxapp/getCode", json=body,
...
def main():
    ...
    for x in raw.replace("&", "\n").splitlines():
        ...
        try:
            ...
            print(f"❌ {e}")
    if not accounts and os.getenv("bjhqbwz", "").strip():
        print("❌ 检测到旧变量 bjhqbwz：本脚本账号来源已改为环境变量 YYB_SERVER（格式：服务器地址@账号ID或OpenID），请迁移配置。")
    if not accounts:
