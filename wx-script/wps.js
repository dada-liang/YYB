...
变量值：YYB服务器地址@账号ID或OpenID，多账号用 & 或换行分隔
      也支持 服务器地址@账号#cookie 或 JSON：{"openid":"","cookie":"","secret":""}（手动入口）
...
function parseAccount(raw) {
  ...
  if (!text) return { openid: "", cookie: "", secret: "", server: "" };
  ...
  for (const sep of ["#", "|"]) {
    if (body.includes(sep)) {
      const [openid, cookie, secret] = body.split(sep);
      return { openid: openid.trim(), cookie: (cookie || "").trim(), secret: (secret || "").trim(), server };
    }
  }

  if (body.includes("wps_sid=") || body.includes("kso_sid=")) return { openid: "", cookie: body, secret: "", server };
  return { openid: body, cookie: "", secret: "", server };
}
...
async function wxServerCode(openid, appid, wechat) {
...
class Task {
  constructor(raw) {
    ...
    // 手动 Cookie/secret 账号沿用原缓存键；YYB 自动登录账号用 server@ref
    this.cacheKey = this.openid && !this.cookies.wps_sid && !this.cookies.kso_sid && account.server
      ? yybCacheKey(account.server, this.openid)
      : this.openid || this.cookies.uid || (account.cookie ? md5(account.cookie).slice(0, 16) : `account_${this.index}`);
    this.wechat = account.server && !account.cookie
      ? new WeChatServer({ url: account.server, appid: ACCOUNT_PLUGIN_APPID, auth: process.env.wx_auth || "" })
      : null;
    this.clockConfig = {};
  }
  ...
  async login() {
    if (!this.wechat) throw new Error("YYB_SERVER 格式无效（应为 服务器地址@账号ID或OpenID），无法自动登录。也可提供手动 Cookie 入口");
    const authCode = await wxServerCode(this.openid, ACCOUNT_PLUGIN_APPID, this.wechat);
