...
class Task {
    ...
    async login() {
        ...
        if (body.success === true) {
            ...
            cache[this.cacheId] = { cookies: this.jar.dump(), updatedAt: new Date().toISOString() };
            writeCache(cache);
            return;
        }
    ...
    async prepareXsrf() {
        ...
        if (token) {
            this.xsrf = token;
            const cache = readCache();
            if (cache[this.cacheId]) { cache[this.cacheId].cookies = this.jar.dump(); writeCache(cache); }
            return true;
        }
