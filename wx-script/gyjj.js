...
class Task {
    constructor(env) {
        ...
        this.userId = "";
        this.userInfo = {};
        this.isLegacyToken = /^Mer|eyJ/i.test(this.accountId) || (!this.raw.includes("@") && this.user.length >= 2);
    ...
    async getOperateData() {
        const { data: loginData } = await this.wechat.getCode(this.yybAccount.ref);
        const code = loginData.data.code;
        let userInfo = {};
        try {
            const { data: profileData } = await this.wechat.getUserInfo(this.yybAccount.ref);
            const result = profileData.data || {};
            const candidate = result.user_info || result.userInfo || result.raw || result;
            userInfo = typeof candidate === "string" ? JSON.parse(candidate) : (candidate || {});
        } catch (e) {}
