...
变量值：
  ...
  2. openid#sId，可直接复用 sId/crossAuth（手动token入口）
  3. JSON：{"openid":"...","sId":"...","remark":"..."}

账号来源：环境变量 YYB_SERVER（服务器地址@账号ID或OpenID）或 huazhu 手动入口
*/
...
async function getWxCode(account) {
  if (!account.server || !account.openid) {
    throw new Error("缺少 YYB 服务器地址或账号（YYB_SERVER 格式：服务器地址@账号ID或OpenID），无法获取微信code；可直接提供 sId 跳过取码");
  }
  const wechat = new WeChatServer({ url: account.server, appid: MINI_APP_ID, auth: WX_AUTH });
  const { data } = await wechat.getCode(account.openid);
...
class Huazhu {
  ...
  async login() {
    ...
    if (!this.account.openid) throw new Error("YYB_SERVER 格式无效（应为 服务器地址@账号ID或OpenID），或直接配置 sId/JSON 手动入口");

    const code = await getWxCode(this.account);
...
async function main() {
  $.checkEnv("YYB_SERVER");
  const manualList = splitAccounts(process.env[ckName]).filter((item) => !$.userList.includes(item));
  const accounts = $.userList.concat(manualList);
  if (!accounts.length) {
