...
*/
const { Env, yybCacheKey } = require("./env");
...
function parseAccount(raw = "") {
  ...
  if (at <= 0) return { server: "", ref: "", openid: "", remark: "" };
  const rawServer = text.slice(0, at);
  ...
  return { server: /^https?:\/\//i.test(value) ? value : `http://${value}`, ref, openid: ref, remark };
}
...
async function getWxCode(account) {
  const wechat = new WeChatServer({ url: account.server, appid: MINI_APPID, auth: process.env.wx_auth || "" });
  const { data } = await wechat.getCode(account.ref);
  return data.data.code;
}

async function loginByCode(account, accountKey) {
  $.log("🔐 正在获取code并登录...");
  ...
  updateCachedToken(accountKey, token);
...
async function getAccessToken(account) {
  ...
  const yybAccount = parseAccount(value);
  const accountKey = yybCacheKey(yybAccount.server, yybAccount.ref);
  const cache = readCache();
  const cachedToken = cache[accountKey]?.token;
  if (cachedToken) {
    $.log(`🔎 使用缓存token校验: ${maskToken(cachedToken)}`);
    if (await validateToken(accountKey, cachedToken)) {
      $.log("✅ 缓存token有效");
      return cachedToken;
    }
  }

  return loginByCode(yybAccount, accountKey);
}

// ================== 核心逻辑 ==================
async function signIn(account, index) {
  let token = await getAccessToken(account);
  const parsedAccount = isToken(account) ? null : parseAccount(account);
  const requestAccount = isToken(account) ? account : yybCacheKey(parsedAccount.server, parsedAccount.ref);
...
!(async () => {
  ...
  let accounts = [];
  if ($.isNode()) {
    const env = process.env.YYB_SERVER || "";
    if (env) {
