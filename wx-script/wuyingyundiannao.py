...
def save_cached_session(account, login_token, session_id):
    cache = read_token_cache()
    cache[account_cache_key(account)] = {"LoginToken": login_token, "SessionId": session_id,
...
def obtain_session(account, index, force=False):
    ...
    if manual and not force:
        ...
        if len(parts) >= 2:
            ...
            if refreshed:
                lt, sid = refreshed
                save_cached_session(account, lt, sid)
...
def run_account(account, index):
    ...
    lt, sid, state = obtain_session(account, index)
    if state != "ok":
    ...
    if session_bad:                       # 会话失效 -> 强制重登重试一次
        ...
        lt, sid, state = obtain_session(account, index, force=True)
        if state != "ok":
            print(f"⚠️ 账号 {index} {state}")
            lines.append(f"⚠️ {state}")
            return "\n".join(lines), False
...
def main():
    ...
    for x in raw.replace("&", "\n").splitlines():
        ...
        try:
            ...
            print(f"❌ {e}")
    if not accounts and os.getenv("wuying", "").strip():
        print("❌ 检测到旧变量 wuying：本脚本账号来源已改为环境变量 YYB_SERVER（格式：服务器地址@账号ID或OpenID），请迁移配置。")
    if not accounts:
    ...
    print("\n=============== 无影云电脑 签到结束 ===============")
    title = f"无影云电脑签到 {ok_count}/{len(accounts)} 成功"
    try:
        send(title, "\n\n".join(summaries))
    except Exception as e:
        print(f"⚠️ 通知发送失败: {e}")
