...
说明：登录仅用 wx.login code（authorize.json 内部做 code2session），YYB Go 取码即可，
...
async function getWxCode(server, openid) {
    let lastError = "";
    for (let attempt = 0; attempt < 4; attempt++) {
        if (attempt) await new Promise((resolve) => setTimeout(resolve, 3000));
        const { status, data } = await axios.post(`${server}/wxapp/getCode`, {
...
class Task {
    ...
    async loginByWxCode() {
        try {
            const code = await getWxCode(this.account.server, this.openid);
