...
class Task {
    ...
    /** 手机号授权 code —— 这家登录接口的必填项，走 YYB /wxapp/getPhoneNumber */
    async getPhoneCode() {
        const { data } = await this.wechat.getPhoneNumber(this.account.ref);
        const result = data.data || {};
        let raw = result.raw || {};
        if (typeof raw === "string") { try { raw = JSON.parse(raw); } catch { raw = {}; } }
        const code = result.phoneCode || result.phone_code || result.code || raw.phoneCode || raw.phone_code || raw.code || "";
        if (!code) throw new Error(`YYB未返回手机号code: ${short(result)}`);
        return code;
    }
