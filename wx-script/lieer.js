...
也支持直接填 token（手动入口），格式：token=<access_token 或 bearer...>
...
class Task {
    constructor(account) {
        this.index = $.userIdx++;
        this.raw = String(account || "").trim();
        this.yyb = parseAccount(this.raw);
        this.account = this.yyb ? this.yyb.ref : this.raw;
        this.remark = this.yyb ? this.yyb.remark : "";
        this.cacheId = this.yyb ? yybCacheKey(this.yyb.server, this.yyb.ref) : this.raw;
        ...
        this.isDirectToken = /^token=/i.test(this.raw) || /^bearer/i.test(this.raw);
        if (this.isDirectToken) this.applyToken({ authorization: this.raw.replace(/^token=/i, "") });
    }

    async run() {
        if (!this.authorization && !this.yyb) {
            $.log(`账号[${this.index}] ❌ YYB_SERVER 格式无效（应为 服务器地址@账号ID或OpenID，或 token= 手动入口）`);
            return;
        }
    ...
    cacheKey() {
        return this.isDirectToken ? `token:${this.authorization.slice(-16)}` : this.cacheId;
    }
