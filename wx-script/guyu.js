...
function parseAccount(raw = "") {
    ...
    if (at <= 0) return { server: "", ref: "", openid: "" };
    ...
    return { server: /^https?:\/\//i.test(value) ? value : `http://${value}`, ref, openid: ref };
}
class Task {
    constructor(env) {
        this.index = $.userIdx++
        this.user = env.split(strSplitor);
        this.account = parseAccount(env);
        this.wechat = new WeChatServer({ url: this.account.server, appid: "wxda948f3be0afc375", auth: process.env.wx_auth || "" });
        this.token = null
        this.wcsid = this.account.openid
        this.isSign = false
    ...
    async run() {
        ...
        try {
            ...
            $.log(`账号[${this.index}] 获取微信code失败:${e.message || e}❌`)
            return
        }
        if (!this.token) {
