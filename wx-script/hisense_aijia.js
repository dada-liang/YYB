...
说明：openid 首次自动登录会通过 YYB 的 /wxapp/getPhoneNumber 获取手机号授权 code。
*/
...
function parseAccount(raw) {
  ...
  if (text.lastIndexOf("@") > 0) {
    const account = parseYYBAccount(text);
    if (!account) return { invalid: true, raw: text };
    return { openid: account.openid, server: account.server, remark: account.remark };
  }
...
class Task {
  constructor(raw) {
    ...
    this.yybAccount = account.server && this.openid ? parseYYBAccount(`${account.server}@${this.openid}`) || {} : {};
    ...
    this.cacheKey = this.openid && !this.accessToken
      ? yybCacheKey(this.yybAccount.server, this.yybAccount.ref)
      : this.openid || this.customerId || (this.accessToken ? md5(this.accessToken).slice(0, 16) : `account_${this.index}`);
  }
  ...
  async getPhoneNumberData() {
    if (!this.wechat) throw new Error("YYB_SERVER 格式无效（应为 服务器地址@账号ID或OpenID），无法获取手机号授权 code");
    ...
    if (!phoneCode) throw new Error(`YYB未返回手机号 code: ${short(result, 500)}`);
