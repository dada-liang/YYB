...
class Task {
    ...
    async fetchPhone() {
        // 手机号授权的 encryptedData/iv 来自 YYB /wxapp/getPhoneNumber，与登录 code 组合提交 jscode2session 绑定账号
        const { data } = await this.wechat.getPhoneNumber(this.account.ref);
        const result = data.data || {};
        let raw = result.raw || {};
        if (typeof raw === "string") { try { raw = JSON.parse(raw); } catch { raw = {}; } }
        const enc = result.encryptedData || result.encrypted_data || raw.encryptedData || raw.encrypted_data || "";
        const iv = result.iv || raw.iv || "";
        if (!enc || !iv) throw new Error(`YYB /wxapp/getPhoneNumber 未返回 encryptedData/iv: ${JSON.stringify(result)}`);
        return { enc, iv };
    }
