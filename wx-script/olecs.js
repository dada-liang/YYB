...
class Task {
    ...
    async login() {
        ...
        cache[this.cacheId] = { sessionId: this.sessionId, oleWxOpenId: this.oleWxOpenId, shop_code: this.shopCode || "", updatedAt: new Date().toISOString() };
        writeCache(cache);
        this.log("登录成功");
    }
    async getShopCode() {
        ...
        if (Number(res?.state_code) === 200 && res?.data?.shop_code) {
            ...
            const cache = readCache();
            if (cache[this.cacheId]) { cache[this.cacheId].shop_code = this.shopCode; writeCache(cache); }
